// Plan view of the Venom Hall's two rooms, so the layout can be judged as a
// drawing rather than as text. Technique copied from tideglass-atlas.js.
const d = window.dev;
...
const out = [];
for (const id of ['mirehaven_hall', 'mirehaven_hall_under']) {
  const sc = new Overworld(state, id, 2, 2, 'down');
  d.game.scenes.replaceAll(sc);
  await d.loadWait(1400);
  const map = sc.map;
  if (!map) { out.push(id + ': no map'); continue; }
  const W = map.width, H = map.height, px = ts.TILE_SIZE * 2;
