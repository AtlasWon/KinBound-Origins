...
test('every warp points at a real map and a walkable tile', () => {
  for (const [id, map] of Object.entries(MAPS)) {
    for (const warp of map.warps ?? []) {
      ...
      assert.ok(!'TtoO#wR[]^|_!CcIK1234567890Ghmbek'.includes(ch),
...
test('every warp sits on a tile the player can actually stand on', () => {
  ...
  const solid = 'TtoO#wR[]^|_!CcIK1234567890Ghmbek';
  for (const [id, map] of Object.entries(MAPS)) {
    for (const warp of map.warps ?? []) {
...
test('the world is one connected graph reachable from the starting town', () => {
  ...
  while (queue.length) {
    ...
    for (const warp of MAPS[id]?.warps ?? []) {
      if (!seen.has(warp.toMap) && MAPS[warp.toMap]) {
        seen.add(warp.toMap);
  ...
  for (const [id, map] of Object.entries(MAPS)) {
    ...
    for (const o of map.objects ?? []) {
      ...
      if (o.kind === 'switch' || o.kind === 'pushable') {
        ...
        assert.ok(!'TtoO#wR[]^|_!CcI1234567890Ghmbek'.includes(ch),
