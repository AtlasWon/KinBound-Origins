// Plan view of the three maps I own (Eastwind Ridge, the waypoint village,
// the coast road) plus their sub-maps, for layout judging at map scale.
// Same technique as routeatlas.js.
const d = window.dev;
...
const MAPS = (window.__ATLAS_MAPS) || ['route_3', 'route_3_whistle', 'tanners_rest', 'route_4', 'route_4_hollow'];
const out = [];
...
for (const id of MAPS) {
  ...
  const res = await fetch('/__shot/' + encodeURIComponent('ridge-' + id), { method: 'POST', body: url });
