// Walk Emberfall.
//
// Forked from tools/shots/tideglass.js, which is forked from act1.js: it
// pathfinds over the map's own collision rather than hardcoding a route, and
// additionally treats the tiles people are standing on as blocked, because a
// city with twenty-eight townsfolk in it has townsfolk standing in doorways
// and a search that walks through them reports a good street as unreachable.
//
// Emberfall is entered by scene replacement rather than by walking in over the
// pass, because the pass is another agent's map and is not wired to this one
// yet. That is a statement about the road, not about the city: everything
// below still walks on the city's own collision, and every door is opened by
// standing in the street and pressing up.
//
...
const enterCity = async (x = 36, y = 4) => {
  d.game.scenes.replaceAll(new Overworld(state, 'emberfall', x, y, 'down'));
...
const town = ow()?.map;
if (!town) return { fatal: 'emberfall did not load', probe: d.probe() };
...
const shots = [
  ...
  ['ef-02-springs', 15, 10],
  ['ef-03-pans', 26, 16],
  ['ef-04-pipestreet', 30, 21],
  ['ef-05-hallsquare', 35, 30],
  ['ef-06-clinicrow', 17, 28],
  ['ef-07-terraces', 20, 39],
  ['ef-08-forgerow', 55, 45],
  ['ef-09-foundry', 62, 40],
  ['ef-10-slag', 58, 53],
  ['ef-11-fissures', 25, 65],
  ['ef-12-templegate', 6, 45],
  ['ef-13-lowertown', 30, 58],
  ['ef-14-mireroad', 74, 53],
];
const out = [];
...
for (const w of doors) {
  if (d.probe().map !== 'emberfall') await enterCity();
  ...
  // A picture of every room, from just inside its own door: an interior that
  // compiles and warps correctly can still be a wall of the wrong material.
  out.push('in-' + w.toMap + ' -> ' + (await d.shoot('ef-in-' + w.toMap.replace('emberfall_', ''), 6, 1)));
  const back = (ow()?.map?.warps || []).find((b) => b.toMap === 'emberfall');
  ...
  await d.loadWait(700);
  clear();
  if (d.probe().map !== 'emberfall') bad.push(`${w.toMap}: the way out landed in ${d.probe().map}`);
}
...
for (const n of (ow()?.map?.npcs || [])) {
  ...
  // Where the person is NOW, not where the map file put them: a third of this
  // city paces or wanders, and walking to the authored tile of somebody who
  // has since strolled two cells north and reporting them as mute is exactly
  // the kind of lie a driver must not tell.
  const live = (ow()?.npcs || []).find((a2) => a2.data?.id === n.id);
  const ax = live?.actor?.tileX ?? n.x;
  const ay = live?.actor?.tileY ?? n.y;
  const spots = [[ax, ay + 1, 'KeyW'], [ax, ay - 1, 'KeyS'], [ax - 1, ay, 'KeyD'], [ax + 1, ay, 'KeyA'],
                 [n.x, n.y + 1, 'KeyW'], [n.x, n.y - 1, 'KeyS'], [n.x - 1, n.y, 'KeyD'], [n.x + 1, n.y, 'KeyA']];
  let spoke = false;
  // Two passes, because a wanderer can step off the tile between the search
  // being planned and the walk finishing. Re-reading their position and going
  // again is the difference between "this person has no lines" and "this
  // person moved", and only one of those is worth reporting.
  for (let pass = 0; pass < 2 && !spoke; pass++) {
    const now = (ow()?.npcs || []).find((a2) => a2.data?.id === n.id);
    const cx = now?.actor?.tileX ?? ax;
    const cy = now?.actor?.tileY ?? ay;
    const tries = pass === 0 ? spots
      : [[cx, cy + 1, 'KeyW'], [cx, cy - 1, 'KeyS'], [cx - 1, cy, 'KeyD'], [cx + 1, cy, 'KeyA']];
    for (const [sx, sy, face] of tries) {
