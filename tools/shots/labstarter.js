// End to end: into the lab, talk to Sorrell, take a starter, walk out, meet Tarin.
const d = window.dev;
...
ow.state.setFlag('met_tarin');
await ow.loadMap(d.game, 'sorrell_lab', 7, 9, 'up');
...
d.key('Enter', 8);
// Sorrell's speech until the starter scene opens.
...
go('down',1);
await d.shoot('e2e-' + tag + '-06-tarin-in', 40);
await d.shoot('e2e-' + tag + '-07-tarin-here', 42);
