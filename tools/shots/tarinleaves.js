...
// through the house, out of the front door, and on through Tarin's departure.
...
// Talk to Mother at (5,4) -- this sets mom_sendoff and puts Tarin outside.
...
const tarin = () => {
  const s = ow();
  const n = s.npcs && s.npcs.find((q) => q.data.id === 'hm_tarin');
...
out.push('greeting over, tarin at ' + (tarin() ? tarin().tileX + ',' + tarin().tileY : 'none'));

let t = 0, leftView = -1, shot = 0;
while (t < 400) {
  if (top().name === 'dialogue') break;
  if (leftView < 0 && tarin() && !onScreen()) leftView = t;
  if (t % 10 === 0) {
    shot++;
